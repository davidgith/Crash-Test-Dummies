/**
 * @file "dashboard.cpp"
 * @brief Implementaion of the Dashboard class.
 *
*/
...
{
  if (!ros::service::waitForService(getFirmwareService, 100))
...
{
  if (!ros::service::waitForService(getCarIdService, 100))
    ROS_WARN_STREAM("Get car id service not availible!");
...
{
  if (!ros::service::waitForService(getSidService, 100))
...
void Dashboard::toggleKinect()
{
  ...
  if (!ros::service::waitForService(toggleKinectService, 100))
...
void Dashboard::toggleUS()
{
  ...
  if (!ros::service::waitForService(toggleUSService, 100))
...
void Dashboard::toggleMotor()
{
  ...
  if (!ros::service::waitForService(toggleMotorService, 100))
...
void Dashboard::toggleDAQ()
{
  ...
  if (!ros::service::waitForService(toggleDAQService, 100))
